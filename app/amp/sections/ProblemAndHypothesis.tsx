...
function Hypothesis({ number, text, className }: { number: number; text: string; className?: string }) {
  return (
    <div className={`flex flex-col  p-8 rounded-2xl gap-5 bg-linear-to-l from-grey-bg-primary to-[#F5F5F500] ${className ?? ""}`}>
      ...
      <div className="text-[16px] font-medium font-sf tracking-[-0.17px] ">
        {text}
      </div>
    </div>
  );
  ...
  return (
    <div className="relative col-start-1 col-span-5 row-start-3  h-80">
      ...
      <div className="absolute top-1/2 -translate-y-1/2 flex flex-col rounded-2xl p-8 gap-5" style={{ background: "linear-gradient(90deg, rgba(0, 126, 255, 0.3) 0%, rgba(245, 245, 245, 0) 60.58%)" }}>
        ...
        <span className="text-[16px] font-sf tracking-[-0.17px] text-grey-text-dark font-medium">{"Текущая модель обработки входящих сообщений\u00A0не\u00A0масштабируется при росте нагрузки\u00A0и\u00A0приводит\u00A0к\u00A0задержкам\u00A0в\u00A0пиковые"} <br /> {"периоды."}</span>
        <span className="text-[16px] font-sf tracking-[-0.17px] text-grey-text-dark font-medium">{"При этом автоматизация воспринимается как"}<br/> {"риск\u00A0из-за\u00A0отсутствия\u00A0прозрачного контроля."}</span>
      </div>
    </div>
  );
}
